...
def plotGASimStatistics(statistics):
  ...
  for j in range(lowEnergyCnt):
    ax1.scatter(x, lm[:, j], s=10, c='r', marker="s")
  ...
  ax1.yaxis.set_ticks(np.arange(ystart, yend, 0.5))
  
  plt.subplots_adjust(left=0.15, bottom=0.11, top=0.98, right=0.98)
...
def readGAStatsFile(gaIter, filePath, prevStats):
  ...
  for line in f:
    ...
    if lineCnt > 2:
      ...
      if edfn > 0 and status == 1 and energy != 0.0 and structCnt < 19:
        structCnt += 1
